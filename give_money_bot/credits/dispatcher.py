...
async def read_num_from_user(message: types.Message, user: User, session: Session) -> None:
    ...
    await message.answer(
        ...
        reply_markup=kb.get_keyboard_users_for_credit(message.from_user.id, value, set(), session),
    )
...
async def prc_callback_choose_users_for_credit(
    ...
    await call.message.edit_reply_markup(
        reply_markup=kb.get_keyboard_users_for_credit(call.from_user.id, value, users, session)
    )
    await call.answer()
