...
dp.register_message_handler(read_num_from_user, check_user)  # Добавляем последним, чтобы обрабатывать сообщение, как новый долг только, когда не прошло по остальным пунктам
