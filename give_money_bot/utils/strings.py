...
class Strings:
    ...
    @staticmethod
    def ASK_FOR_DEBTORS(value: int, info: str, negative=False) -> str:
        if negative:
            return f"Кому ты должен {abs(value)} руб?\n{info}"
        return f"Кто тебе должен {value} руб?\n{info}"

    @staticmethod
    def SAVE_CREDIT(value: int, usernames: List[str], info: str, negative=False) -> str:
        if negative:
    ...
    @staticmethod
    def ANNOUNCE_NEW_CREDIT(value: int, username: str, info: str, negative=False) -> str:
        if negative:
