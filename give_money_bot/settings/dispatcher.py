...
from give_money_bot.db.models import User, ShowTypes
from give_money_bot.settings import keyboards as kb
from give_money_bot.settings.keyboards import EditVisibilityCallback, Action
from give_money_bot.settings.states import SettingsStates, EditVisibilityData, EditVisibilityUser, PAGE_MAX_USERS
...
async def edit_user_visibility(message: types.Message, session: Session, state: FSMContext, user: User) -> None:
    ...
    edit_visibility_data = EditVisibilityData(users=edit_visibility_users, users_list=users_list, page=1, pages_total=pages)
...
async def edit_user_visibility_user_click(call: CallbackQuery, state: FSMContext, callback_data: EditVisibilityCallback) -> None:
    state_data = await state.get_data()
    edit_visibility_data: EditVisibilityData = EditVisibilityData.parse_raw(state_data.get("edit_visibility"))

    edit_visibility_data.users[callback_data.user_id].vision = (edit_visibility_data.users[callback_data.user_id].vision + 1) % 3
...
async def edit_user_visibility_save_click(call: CallbackQuery, session: Session, state: FSMContext) -> None:
    ...
    for user in edit_visibility_data.users_list:
        db.update_user_vision(session, user_id=call.from_user.id, show_user_id=user, show_type=edit_visibility_data.users[user].vision)
...
router.callback_query.register(edit_user_visibility_user_click, EditVisibilityCallback.filter(F.action == Action.user))
router.callback_query.register(edit_user_visibility_right_click, EditVisibilityCallback.filter(F.action == Action.right))
router.callback_query.register(edit_user_visibility_left_click, EditVisibilityCallback.filter(F.action == Action.left))
router.callback_query.register(edit_user_visibility_cancel_click, EditVisibilityCallback.filter(F.action == Action.cancel))
router.callback_query.register(edit_user_visibility_save_click, EditVisibilityCallback.filter(F.action == Action.save))
