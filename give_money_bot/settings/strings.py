...
class Strings:
    ...
    ask_for_new_number = (
        "Напиши мне номер, на который тебе надо возвращать долг. "
        "Так же можешь уточнить банк, на который надо отправлять деньги"
    )
    ...
    edit_visibility_description = (
        ...
        f"{vision_emojis[1]} - означает, что ты будешь видеть этого пользователя только после "
        f"нажатия дополнительной кнопки \"Показать еще\"\n"
