...
cancel_crt_credit_inline = InlineKeyboardButton("Отмена", callback_data=CALLBACK.cancel_crt_credit)
return_credit_inline = InlineKeyboardButton("Вернуть выбранные долги", callback_data=CALLBACK.return_credits)
cancel_return_credit_inline = InlineKeyboardButton("Отмена", callback_data=CALLBACK.cancel_return_credits)


def get_check_markup(credit_id: Union[str, int], value: bool) -> InlineKeyboardMarkup:
    markup = InlineKeyboardMarkup()
    if value:
        markup.add(InlineKeyboardButton(f"Все верно{Emoji.TRUE}", callback_data=CALLBACK.check_return_approve))
        markup.add(InlineKeyboardButton(f"Это не так{Emoji.FALSE}", callback_data=CALLBACK.check_return_reject))
        markup.add(InlineKeyboardButton("Сохранить", callback_data=check_return_data.new(credit_id, "1")))
    else:
        markup.add(InlineKeyboardButton(f"Все верно{Emoji.FALSE}", callback_data=CALLBACK.check_return_approve))
        markup.add(InlineKeyboardButton(f"Это не так{Emoji.TRUE}", callback_data=CALLBACK.check_return_reject))
        markup.add(InlineKeyboardButton("Сохранить", callback_data=check_return_data.new(credit_id, "0")))
    return markup
...
def get_keyboard_users_for_credit(for_user_id: int, value: int, users: Set[int]) -> InlineKeyboardMarkup:
    ...
    for user_id in USERS.keys():
        if user_id != for_user_id:
            ...
            markup.add(InlineKeyboardButton(text, callback_data=user_choose_data.new(user_id, has_mark)))
